 ...
 * grants, plus the controls that resolve it. Even levels roll the class hit die
 * for HP, odd levels roll 2d6 on the class talent table — one roll per level,
 * sealed as soon as it lands and reversible through "Anular".
 */
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col">
            ...
            <span className="font-mono mt-2 flex items-center gap-1.5 text-[9px] tracking-[0.14em] text-[var(--bone-dim)]">
              <HugeiconsIcon icon={isHp ? FavouriteIcon : SparklesIcon} size={12} strokeWidth={2} />
              {isHp ? 'VITALIDADE' : 'TALENTO'}
            </span>
          </div>
          ...
          >
            {sealed ? (
              ...
              <span className="font-heading text-[20px] leading-none font-black">?</span>
            )}
          </div>
        </div>
        ...
        <p className="font-sans text-[13px] leading-snug text-[var(--bone-muted)] italic">
          {isHp
            ? `Nível par — role o dado de vida da classe e some o modificador de CON ao seu PV máximo.`
        ...
        {/* Receipt for a sealed level */}
        {sealed && entry && <LevelReceipt entry={entry} hpMax={hpMax} />}

        {/* Actions */}
        {locked ? (
          <p className="font-mono text-[9px] tracking-[0.1em] text-[var(--muted-foreground)]">
            ✦ Alcance o nível {level} para desbloquear esta recompensa.
          </p>
        ) : sealed ? (
          <Button
            variant="outline"
          ...
          <div className="flex gap-1.5">
            ...
            {characterLevel < MAX_LEVEL && (
              <Button
                ...
                title={sealed ? undefined : 'Resolva a recompensa deste nível antes de avançar'}
                className={cn(
...
function StatTile({
  ...
  return (
    <div className="bg-secondary flex flex-col gap-1 border border-[var(--border)] px-3 py-2">
      ...
      <span
        className={cn(
          'font-heading truncate text-[18px] leading-none',
          tone === 'good' && 'text-[var(--chart-1)]',
          tone === 'bad' && 'text-[var(--destructive)]',
          tone === 'neutral' && 'text-[var(--parchment-pale)]',
        )}
      >
        {value}
      </span>
...
function LevelReceipt({ entry, hpMax }: { entry: LevelEntry; hpMax: number }) {
  if (entry.kind === 'hp') {
    return (
      <div className="flex flex-col gap-1 border border-[var(--chart-1)]/40 bg-[var(--chart-2)]/15 px-3 py-2.5">
        ...
        <span className="font-mono text-[8.5px] text-[var(--muted-foreground)]">
          {entry.hpRoll} (d{entry.hpDie}) {(entry.conMod ?? 0) >= 0 ? '+' : ''}
          {entry.conMod ?? 0} (CON) → PV máximo {hpMax}
        </span>
      </div>
    )
  }
