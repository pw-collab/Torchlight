...
export function StepArchetype({ classId, archetypeId, onChange }: Props) {
  ...
  const options: CardOption[] = archetypes.map(a => ({
    id: a.id,
    title: a.name,
    caption: cls?.name ?? 'Arquétipo',
    glyph: a.glyph,
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <CardPicker
        ...
        onSelect={onChange}
        actionLabel="Escolher este arquétipo"
        chosenLabel="Arquétipo escolhido"
        emptyHint={
          classId
            ...
            <>
                    ...
                    {archetype.kit.map(kit => {
                      ...
                      return (
                        <li key={name}>
                          ...
                          {note && (
                            <span className="text-muted-foreground block text-[12px] leading-snug italic">
                              {note}
                            </span>
                          )}
                        </li>
              ...
              {archetype.questions && archetype.questions.length > 0 && (
                <DetailBlock label="Perguntas de arquétipo">
                  <p className="text-muted-foreground mb-1 text-[11px] italic">
