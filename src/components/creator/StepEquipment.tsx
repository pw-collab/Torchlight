          ...
          <div className="mb-[5px] flex items-center justify-between">
            <span
              className={cn(
                ...
                encumbered ? 'text-[var(--destructive)]' : 'text-muted-foreground',
              )}
            >
              Slots de Carga
            </span>
            <span
              className={cn(
                'font-mono text-[9px]',
                encumbered ? 'text-[var(--destructive)]' : 'text-[var(--foreground)]',
              )}
            >
              {used} / {max}
            </span>
            ...
            {granted.map(item => (
              ...
              >
                <ItemContent className="gap-0">
                  <ItemTitle className="text-xs font-normal text-[var(--foreground)]">
                    {item.name}
                  </ItemTitle>
                </ItemContent>
                <Badge
          ...
          <div className="flex max-h-[320px] flex-col gap-1 overflow-y-auto pr-1">
            {catalogList.map(item => (
              <button
                ...
                className="tactile flex cursor-pointer items-center gap-2.5 rounded-sm border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-left transition-colors duration-150 hover:border-[var(--muted-foreground)]"
              >
                <span className="min-w-0 flex-1">
                  <span className="block text-[11.5px] text-[var(--foreground)]">
                    {item.name}
                  </span>
                ...
                {item.acBonus != null && (
                  <span className="font-mono shrink-0 text-[9px] text-[var(--chart-2)]">
                    CA {item.acBonus}
                  </span>
                )}
                ...
                <span className="font-mono shrink-0 text-[12px] text-[var(--primary)]">+</span>
              </button>
            ))}
      ...
      <StepSection title="Áreas de Conhecimento">
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    ka.bonus >= 0 ? 'text-[var(--chart-2)]' : 'text-[var(--destructive)]',
                  )}
                >
        ...
        <div className="flex gap-1.5">
          <Input
            ...
            className="h-auto flex-1 rounded-sm border-[var(--border)] bg-[var(--card)] px-2.5 py-2 text-xs text-[var(--foreground)] focus-visible:border-[var(--primary)]"
          />
          <div className="flex items-center gap-1">
            ...
            <span
              className={cn(
                ...
                kaBonus >= 0 ? 'text-[var(--chart-2)]' : 'text-[var(--destructive)]',
              )}
            >
          ...
          <Button
            variant="outline"
            onClick={addArea}
            className="h-auto rounded-sm border-[var(--border)] bg-[var(--muted)] px-3 py-2 text-[9px] tracking-[0.1em] text-[var(--foreground)]"
          >
            + Add
          </Button>
        </div>
...
const BONUS_BTN_CLASS =
  'font-mono rounded-sm border-[var(--border)] bg-[var(--card)] text-xs text-[var(--foreground)]'
